...
class ComprasExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents
{
    protected $produtos;
    ...
    public function map($row): array
    {
        ...
        return [
            Carbon::parse($row['data_entrada'])->format('d/m/Y'),
            $row['nomeProduto'],
            ($row['tamanho'] ?? 0) . " {$unidadeFormatada}{$plural}{$peso_liquido}",
            trim($row['nomeFornecedor']),
