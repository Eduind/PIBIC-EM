<x-layout.layout-secundario>
    ...
    <div class="form-container">
        ...
        <div class="botao">
            <form action="{{route('novaMarcaSubmit')}}" method="post">
                @csrf
                <input type="text" name="text_name" value="{{old('text_name')}}" placeholder="Nome da marca" required>
                @error('text_name')
                        <small class="text-danger d-block ms-2">{{ $message }}</small>
                @enderror
                <input type="text" name="text_descricao" value="{{old('text_descricao')}}" placeholder="Descrição" required>
